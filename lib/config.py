...
sentinel_cfg = MonsterNodeConfig.tokenize(sentinel_config_file)
sentinel_version = "1.1.0"
min_monsternoded_proto_version_with_sentinel_ping = 70207


def get_monsternode_conf():
    home = os.environ.get('HOME')

    monsternode_conf = os.path.join(home, ".monsternodecore/monsternode.conf")
    if sys.platform == 'darwin':
        monsternode_conf = os.path.join(home, "Library/Application Support/MonsterNodeCore/monsternode.conf")

    monsternode_conf = sentinel_cfg.get('monsternode_conf', monsternode_conf)

    return monsternode_conf
...
monsternode_conf = get_monsternode_conf()
network = get_network()
db = get_db_conn()
