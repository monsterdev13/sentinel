...
class GovernanceClass(object):
    ...
    def vote(self, monsternoded, signal, outcome):
        return self.go.vote(monsternoded, signal, outcome)

    # pass thru to GovernanceObject#voted_on
    def voted_on(self, **kwargs):
        return self.go.voted_on(**kwargs)

    def vote_validity(self, monsternoded):
        if self.is_valid():
            printdbg("Voting valid! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(monsternoded, models.VoteSignals.valid, models.VoteOutcomes.yes)
        else:
            printdbg("Voting INVALID! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(monsternoded, models.VoteSignals.valid, models.VoteOutcomes.no)
    ...
    def list(self):
        ...
        # return a dict similar to monsternoded "gobject list" output
    ...
    def monsternoded_serialise(self):
        import monsternodelib
        return monsternodelib.SHIM_serialise_for_monsternoded(self.serialise())
