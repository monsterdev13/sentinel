...
def is_valid_monsternode_address(address, network='mainnet'):
    ...
    monsternode_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid MonsterNode address
        return False

    if (address_version != monsternode_version):
        return False
...
# shims 'til we can fix the monsternoded side
def SHIM_serialise_for_monsternoded(sentinel_hex):
    from models import DASHD_GOVOBJ_TYPES
    # unpack
    obj = deserialise(sentinel_hex)

    # shim for monsternoded
    govtype = obj[0]

    # add 'type' attribute
    obj[1]['type'] = DASHD_GOVOBJ_TYPES[govtype]

    # superblock => "trigger" in monsternoded
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # monsternoded expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    monsternoded_hex = serialise(obj)
    return monsternoded_hex


# shims 'til we can fix the monsternoded side
def SHIM_deserialise_from_monsternoded(monsternoded_hex):
    from models import DASHD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(monsternoded_hex)

    # shim from monsternoded
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('monsternode.conf')
