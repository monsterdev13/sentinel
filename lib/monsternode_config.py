...
class MonsterNodeConfig():

    @classmethod
    def slurp_config_file(self, filename):
        # read monsternode.conf config but skip commented lines
        f = io.open(filename)
        lines = []
        for line in f:
            if re.match('^\s*#', line):
        ...
        # data is monsternode.conf without commented lines
        data = ''.join(lines)
    ...
    def get_rpc_creds(self, data, network='mainnet'):
        # get rpc info from monsternode.conf
        match = re.findall(r'rpc(user|password|port)=(.*?)$', data, re.MULTILINE)

        # python >= 2.7
        creds = {key: value for (key, value) in match}

        # standard MonsterNode defaults...
        default_port = 6883 if (network == 'mainnet') else 16883

        # use default port for network if not specified in monsternode.conf
        if not ('port' in creds):
            creds[u'port'] = default_port

        # convert to an int if taken from monsternode.conf
