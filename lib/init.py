...
def has_monsternode_conf():
    import config
    import io

    valid_monsternode_conf = False

    # ensure monsternode_conf exists & readable
    #
    # if not, print a message stating that MonsterNode Core must be installed and
    # configured, including JSONRPC access in monsternode.conf
    try:
        f = io.open(config.monsternode_conf)
        valid_monsternode_conf = True
    except IOError as e:
        print(e)

    return valid_monsternode_conf
...
def main():
    ...
    if not has_monsternode_conf():
        print("MonsterNodeCore must be installed and configured, including JSONRPC access in monsternode.conf")
        sys.exit(1)
