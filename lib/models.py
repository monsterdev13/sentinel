...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, monsternoded):
        golist = monsternoded.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_monsternoded(monsternoded, item)
    ...
    @classmethod
    def import_gobject_from_monsternoded(self, monsternoded, rec):
        import decimal
        ...
        # shim/monsternoded conversion
        object_hex = monsternodelib.SHIM_deserialise_from_monsternoded(object_hex)
        objects = monsternodelib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from monsternoded...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from monsternoded, with every run
        ...
        # get/create, then sync payment amounts, etc. from monsternoded - MonsterNoded is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(monsternoded)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from monsternoded! %s" % e)
            govobj.vote_delete(monsternoded)
            return (govobj, None)
    ...
    def vote_delete(self, monsternoded):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(monsternoded, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, monsternoded, signal, outcome):
        import monsternodelib
        ...
        output = monsternoded.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = monsternodelib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(monsternoded, signal)

    def sync_network_vote(self, monsternoded, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = monsternoded.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 monsternode addr, non-multisig
            if not monsternodelib.is_valid_monsternode_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid MonsterNode address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (monsternodelib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/MonsterNodeDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import monsternodelib
        obj_data = monsternodelib.SHIM_serialise_for_monsternoded(self.serialise())
    ...
    def prepare(self, monsternoded):
        try:
            object_hash = monsternoded.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not monsternodelib.is_valid_monsternode_address(addr, config.network):
    ...
    def hash(self):
        import monsternodelib
        return monsternodelib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    @classmethod
    def active(self, monsternoded):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - monsternoded.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, monsternoded):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - monsternoded.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, monsternoded):
        now = int(time.time())
        return (self.created_at < (now - monsternoded.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, monsternoded):
        if self.is_expired(monsternoded):
            return False

        return True

    def is_deletable(self, monsternoded):
        if self.is_expired(monsternoded):
            return True
