"""
monsternoded JSONRPC interface
"""
import sys
import os
...
class MonsterNodeDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some monsternoded methods
    ...
    @classmethod
    def from_monsternode_conf(self, monsternode_dot_conf):
        from monsternode_config import MonsterNodeConfig
        config_text = MonsterNodeConfig.slurp_config_file(monsternode_dot_conf)
        creds = MonsterNodeConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = monsternodelib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = monsternodelib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        DO NOT CALL DIRECTLY if you don't monsternode a "Oh Noes." exception.
        """
        current_block_height = self.rpc_command('getblockcount')
        ...
        future_seconds = monsternodelib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_monsternoded_proto_version_with_sentinel_ping)
